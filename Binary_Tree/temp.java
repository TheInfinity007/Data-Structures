...
class Main
{
    ...
    static void printAllPaths(Node root) 
    {
        ...
        stk.push(mainObj.new Temp(root, 0, String.valueOf(root.data)));
        Node node = root.left;
        Temp obj = stk.peek();
        while(true)
        {
            while(node != null)
            {
                stk.push(mainObj.new Temp(node, obj.distance+1, obj.toString()+ " " + String.valueOf(node.data)));
                node = node.left;
                obj = stk.peek();
            }
            if(stk.size() == 0)
